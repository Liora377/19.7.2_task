...
class PetFriends:
    ...
    def get_api_key(self, email, password):
        ...
        с уникальным ключом пользователя, найденного по указанным email
        и паролю'''
    ...
    def get_list_of_pets(self, auth_key, filter): #возвращает список питомцев
        '''Метод делает запрос к API сервера и возвращает статус запроса и результат
    ...
    def add_photo_of_pet(self, auth_key: json, pet_id: str, pet_photo: str) -> json:
        """Метод отправляет (постит) на сервер в качестве данных только фото для имеющегося питомца
